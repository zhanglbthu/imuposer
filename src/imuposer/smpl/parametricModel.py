...
class ParametricModel:
    ...
    def __init__(self, official_model_file: str, use_pose_blendshape=False, device=torch.device('cpu')):
        ...
        self._J_regressor = torch.from_numpy(data['J_regressor'].toarray()).float().to(device) # [24, 6890]：vertex to joint
        self._skinning_weights = torch.from_numpy(data['weights']).float().to(device) # [6890, 24]: joint to vertex
        self._posedirs = torch.from_numpy(data['posedirs']).float().to(device) # [6890, 3, 207]：pose to vertex
        self._shapedirs = torch.from_numpy(np.array(data['shapedirs'])).float().to(device) # [6890, 3, 10]: shape to vertex
        self._v_template = torch.from_numpy(data['v_template']).float().to(device) # [6890, 3]
        self._J = torch.from_numpy(data['J']).float().to(device) # [24, 3]
        self.face = data['f'] # [13776, 3]
